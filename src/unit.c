...
static const char *ldargs_template[] =
  {
    ...
    /* It is unknown which of these two files will be present, so for
       now we assume that it is crt1.o. */
#if 0
    ARCHITECTURE_DIR "/crt0.o"
#else
    ARCHITECTURE_DIR "/crt1.o"
#endif

    /* If we can't do this on our own, we just run the same compiler
       that created us. */
